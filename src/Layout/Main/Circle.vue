<template>
  <Dialog v-model="visible" @closed="closed" height="280px" width="400px">
    <div class="page-row">
      <el-upload ref="uploadRef" :http-request="httpRequest" class="avatar-uploader" :auto-upload="false" :show-file-list="false" :on-change="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
        <img v-if="imageUrl" :src="imageUrl" class="avatar" />
        <svg v-else class="avatar" viewBox="0,0,257,344" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
          <defs>
            <clipPath id="user-clip1">
              <rect x="382" y="195" width="257" height="344"></rect>
            </clipPath>
            <linearGradient x1="351.462" y1="233.56" x2="669.496" y2="500.422" gradientUnits="userSpaceOnUse" spreadMethod="reflect" id="user-fill3">
            ...
            <linearGradient x1="351.462" y1="233.56" x2="669.496" y2="500.422" gradientUnits="userSpaceOnUse" spreadMethod="reflect" id="user-fill4">
              <stop offset="0" stop-color="#A964C8"></stop>
              <stop offset="0.35" stop-color="#A964C8"></stop>
              <stop offset="0.87" stop-color="#2D8AD5"></stop>
              <stop offset="1" stop-color="#2D8AD5"></stop>
          ...
          <g clip-path="url(#user-clip1)" transform="translate(-382 -195)">
            <path d="M637.755 433.872C642.215 515.221 579.577 537.983 508.011 537.983 436.444 537.983 376.676 507.833 383.513 437.11 383.109 425.234 389.59 414.133 398.634 409.891 413.82 402.768 444.753 402.936 507.484 402.997 570.214 403.058 609.164 402.279 621.521 407.947 633.878 413.614 638.011 424.609 637.755 433.872Z" fill="url(#user-fill3)" fill-rule="evenodd"></path>
            <path d="M422 285C422 235.847 461.623 196 510.5 196 559.377 196 599 235.847 599 285 599 334.153 559.377 374 510.5 374 461.623 374 422 334.153 422 285Z" fill="url(#user-fill4)" fill-rule="evenodd"></path>
          </g>
        </svg>
      </el-upload>
    </div>
    ...
    <el-col :span="24" class="n-col n-footer">
      <el-button type="reset" @click="closed">关闭</el-button>
      <el-button type="primary" @click="saveUserInfo">设置</el-button>
    </el-col>
  </Dialog>
</template>
...
const getUserAvater = () => {
  ...
  if (userInfo.value) {
    ...
    userName.value = users.userName || '微芒计划'
  }
}

const updateUserInfo = (params1) => {
  updateObject('_User', params1).then((res) => {
...
const saveUserInfo = () => {
  if (!haveFile.value) {
    ...
    params1.userName = userName.value
...
const httpRequest = (file) => {
  ...
  uploadFile(params).then((res) => {
    ...
    let params1 = getUserInfoId()
    params1.imageUrl = res.attributes.url
    params1.userName = userName.value
    updateUserInfo(params1)
  })
}
...
.avatar-uploader {
  ...
  img {
    ...
    width: 120px;
    border-radius: 50%;
    border: 2px solid rgba(255, 166, 0, 0.607);
    padding: 3px;
    background: #1111110f;
    object-fit: cover;
  }
